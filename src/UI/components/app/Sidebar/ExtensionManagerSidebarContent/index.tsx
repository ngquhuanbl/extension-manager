...
import ExtensionItem, { Props as Data } from "./ExtensionItem";

const ExtensionManagerSidebarContent: React.FC = () => {
  ...
  return (
    <VStack spacing="24px" padding="16px">
      {data.map((item) => (
        <ExtensionItem key={item.id} {...item} />
      ))}
    </VStack>
  );
};

export default ExtensionManagerSidebarContent;
